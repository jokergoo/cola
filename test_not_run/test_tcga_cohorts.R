...
if(is_methylation) {
	q(save = "no")
	res_list = run_all_consensus_partition_methods(data, max_k = 8, top_n = c(10000, 20000, 30000), mc.cores = ncore,
		scale_rows = FALSE)
} else {
	data = adjust_matrix(data)
	res_list = run_all_consensus_partition_methods(data, max_k = 8, mc.cores = ncore)
}
...
# 	cmd = qq("module load R/3.3.1; Rscript /home/guz/project/development/cola/test_not_run/test_tcga_cohorts.R --file @{f} --ncore 4")
