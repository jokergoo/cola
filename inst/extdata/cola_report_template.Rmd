...
```{r, echo = FALSE, message = FALSE}
message("density heatmap for global distribution in all samples")
```
...
```{r, echo = FALSE, message = FALSE}
message("ECDF from all methods.")
```

```{r collect-plots, fig.width = fs*n_partition_method, fig.height = fs*n_top_method}
...
```{r, echo = FALSE, message = FALSE}
message("Consensus heamtaps from all methods")
```
...
n_partition_method = length(res_list@partition_method)
for(k in all_k) { -%>
knitr_add_tab_item('collect_plots(res_list, k = <%=k%>, fun = consensus_heatmap)', 'k = <%=k%>', opt = 'fig.width = <%=fs*n_partition_method%>, fig.height = <%=fs*n_top_method%>')
<% } -%>
knitr_insert_tabs()
```
...
```{r, echo = FALSE, message = FALSE}
message("Membership heamtaps from all methods")
```

```{r, results = "asis", echo = FALSE, include = TRUE}
<% for(k in all_k) { -%>
knitr_add_tab_item('collect_plots(res_list, k = <%=k%>, fun = membership_heatmap)', 'k = <%=k%>', opt = 'fig.width =<%=fs*n_partition_method%>, fig.height = <%=fs*n_top_method%>')
<%}-%>
knitr_insert_tabs()
```

```{r, echo = FALSE, message = FALSE}
message("Look for signatures from all methods")
```
...
```{r, results = "asis", echo = FALSE, include = TRUE}
<% for(k in all_k) { -%>
knitr_add_tab_item('collect_plots(res_list, k = <%=k%>, fun = get_signatures)', 'k = <%=k%>', opt = 'fig.width = <%=fs*n_partition_method%>, fig.height = <%=fs*n_top_method%>')
<%}-%>
knitr_insert_tabs()
```
...
```{r, echo = FALSE, message = FALSE}
message("Collect classifications from all methods")
```
...
```{r, echo = FALSE, message = FALSE}
message("Compare top row methods")
```
