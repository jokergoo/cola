...
```{r, echo = FALSE, message = FALSE}
message(qq("* making empirical cumulative distribution curves for all methods"))
```
...
n_method = n_top_value_method * n_partition_method
for(k in all_k) { -%>
message("  - consensus heamtaps for all methods (k = <%=k%>)")
...
```{r, results = "asis", echo = FALSE, include = TRUE}
<% for(k in all_k) { -%>
message("  - membership heamtap for all methods (k = <%=k%>)")
...
```{r, results = "asis", echo = FALSE, include = TRUE}
<% for(k in all_k) { -%>
message("  - signature heamtap for all methods (k = <%=k%>)")
...
```{r, results = "asis", echo = FALSE, include = TRUE}
<% for(k in all_k) { -%>
message("  - classifications from all methods (k = <%=k%>)")
...
top_rows_overlap(res_list, top_n = <%= min(res_list@list[[1]]@top_n) %>, method = "correspondance")
```
...
```{r, results = "asis", echo = FALSE, include = TRUE}
<% for(top_n in res_list@list[[1]]@top_n) { -%>
message("  - comparing top row methods by heatmap (top_n=<%=top_n%>)")
