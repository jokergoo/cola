Cola report
==================

**Date**: `r Sys.Date()`

----------------------------------------------------------------

<style type='text/css'>
<%
css = paste(readLines(paste0(TEMPLATE_DIR, "/custom.css")), collapse = "\n") -%>
<%= css %>
</style>

```{r, echo = FALSE, message = FALSE}
library(knitr)
library(markdown)
options(markdown.HTML.options = setdiff(c(getOption("markdown.HTML.options"), "toc"), "base64_images"))
options(width = 100)
opts_chunk$set(
	comment = "#>",
	fig.path = "figure_cola/",
	warning = FALSE,
	message = FALSE
)
library(ComplexHeatmap)
```

<% res_list = object -%>

## Summary

All available functions which can be applied to `res_list` object.

<% same_var_name = var_name == "res_list" -%>

```{r, eval = <%= ifelse(same_var_name, '1:2','2:3')%>, echo = FALSE}
<% if(!same_var_name) { -%>
res_list = <%=var_name%>
<% } -%>
res_list = object
res_list
```

```{r, eval = FALSE, echo = TRUE}
<% if(!same_var_name) { -%>
res_list = <%=var_name%>
<% } -%>
res_list
```

The call for `run_all_consensus_partition_methods()`:

```{r, echo = FALSE}
print(res_list@calling)
```

Dimension of the input matrix:

```{r}
mat = get_matrix(res_list)
dim(mat)
```

Global distribution for each sample:

```{r, echo = FALSE, message = FALSE}
message(qq("* density heatmap of global distribution in each sample (@{ncol(mat)} samples)"))
```

```{r density-heatmap}
library(ComplexHeatmap)
<% if(is.null(res_list@list[[1]]@anno)) { -%>
densityHeatmap(mat, ylab = "value")
<% } else { -%>
densityHeatmap(mat, anno = HeatmapAnnotation(df = res_list@list[[1]]@anno, 
	col = res_list@list[[1]]@anno_col), ylab = "value")
<% } -%>
```

```{r, echo = FALSE, message = FALSE}
n_top_value_method = length(res_list@top_value_method)
n_partition_method = length(res_list@partition_method)
n_method = n_top_value_method * n_partition_method
message(qq("* top rows are extracted by '@{paste(res_list@top_value_method, collapse = ', ')}' methods"))
message(qq("* subgroups are detected by '@{paste(res_list@partition_method, collapse = ', ')}' methods"))
message(qq("* finally there are @{n_method} combinations of methods"))
```

```{r, echo = FALSE, message = FALSE}
message("* guessing best k for each method")
```

Best `k` (number of partitions) for each combination of top methods and partition methods.

```{r, eval = FALSE}
guess_best_k(res_list)
```

```{r, echo = FALSE}
tb = guess_best_k(res_list)
kable(tb)
```

```{r, echo = FALSE}
fs = min(c(3*n_top_value_method, 14))/n_partition_method
```

CDF of consensus matrix for all methods:

```{r, echo = FALSE, message = FALSE}
message(qq("* making ecdf curves for all methods (@{n_method} plots)"))
```

```{r collect-plots, fig.width = fs*n_partition_method, fig.height = fs*n_top_value_method}
collect_plots(res_list, fun = plot_ecdf)
```

```{r, echo = FALSE, message = FALSE}
message(qq("* making consensus heatmaps for all methods"))
```

Consensus heatmaps for all methods:

```{r, results = "asis", echo = FALSE, include = TRUE}
<% all_k = res_list@list[[1]]@k
fs = min(c(3*length(res_list@top_value_method), 14))/length(res_list@top_value_method)
n_top_value_method = length(res_list@top_value_method)
n_partition_method = length(res_list@partition_method)
n_method = n_top_value_method * n_partition_method
for(k in all_k) { -%>
message("  - consensus heamtaps for all methods (k=<%=k%>, <%=n_method%> plots)")
knitr_add_tab_item('collect_plots(res_list, k = <%=k%>, fun = consensus_heatmap)', 'k = <%=k%>', opt = 'fig.width = <%=fs*n_partition_method%>, fig.height = <%=fs*n_top_value_method%>')
<% } -%>
knitr_insert_tabs()
```

```{r, echo = FALSE, message = FALSE}
message(qq("* making membership heatmaps for all methods"))
```

Membership heatmaps for all methods:

```{r, results = "asis", echo = FALSE, include = TRUE}
<% for(k in all_k) { -%>
message("  - membership heamtap for all methods (k=<%=k%>, <%=n_method%> plots)")
knitr_add_tab_item('collect_plots(res_list, k = <%=k%>, fun = membership_heatmap)', 'k = <%=k%>', opt = 'fig.width =<%=fs*n_partition_method%>, fig.height = <%=fs*n_top_value_method%>')
<%}-%>
knitr_insert_tabs()
```

```{r, echo = FALSE, message = FALSE}
message(qq("* making signature heatmaps for all methods"))
```

Signature heatmaps for all methods:

```{r, results = "asis", echo = FALSE, include = TRUE}
<% for(k in all_k) { -%>
message("  - signature heamtap for all methods (k=<%=k%>, <%=n_method%> plots)")
knitr_add_tab_item('collect_plots(res_list, k = <%=k%>, fun = get_signatures)', 'k = <%=k%>', opt = 'fig.width = <%=fs*n_partition_method%>, fig.height = <%=fs*n_top_value_method%>')
<%}-%>
knitr_insert_tabs()
```

```{r, echo = FALSE, message = FALSE}
message(qq("* getting statistics for all methods"))
```

Get statistics for all methods:

```{r, results = "asis", echo = FALSE, include = TRUE}
<% for(k in all_k) { -%>
knitr_add_tab_item('get_stat(res_list, k = <%=k%>)', 'k = <%=k%>')
<%}-%>
knitr_insert_tabs()
```


```{r, echo = FALSE, message = FALSE}
message(qq("* collecting classifications for all methods"))
```

Collect partitions from all methods:

```{r, results = "asis", echo = FALSE, include = TRUE}
<% for(k in all_k) { -%>
message("  - collect classifications from all methods (k=<%=k%>)")
knitr_add_tab_item('collect_classes(res_list, k = <%=k%>)', 'k = <%=k%>', opt = 'fig.width = 14, fig.height = 8')
<%}-%>
knitr_insert_tabs()
```

```{r, echo = FALSE, message = FALSE}
message(qq("* overlaping top rows from different top value methods"))
```

Overlap of top rows in different top methods:

```{r, results = "asis", echo = FALSE, include = TRUE}
<% for(top_n in res_list@list[[1]]@top_n) { -%>
knitr_add_tab_item('top_rows_overlap(res_list, top_n = <%=top_n%>)', 'top_n = <%=top_n%>', opt = 'fig.width = 5, fig.height = 5')
<%}-%>
knitr_insert_tabs()
```

Also visualize the correspondance of rankings between different top methods:

```{r top-row-overlap, fig.width = 14, fig.height = 8}
top_rows_overlap(res_list, top_n = <%= min(res_list@list[[1]]@top_n) %>, type = "correspondance")
```

Heatmaps for the top rows:

<% 
n_top_value_method = length(res_list@top_value_method)
if(n_top_value_method <= 4) {
	fig_width = 14/4 * n_top_value_method
	fig_height = 14/4
} else {
	fig_width = 14
	fig_height = 14/n_top_value_method
}
-%>

```{r, results = "asis", echo = FALSE, include = TRUE}
<% for(top_n in res_list@list[[1]]@top_n) { -%>
message("  - compare top row methods by heatmap (top_n=<%=top_n%>)")
knitr_add_tab_item('top_rows_heatmap(res_list, top_n = <%=top_n%>)', 'top_n = <%=top_n%>', opt = 'fig.width = <%=fig_width%>, fig.height = <%=fig_height%>')
<%}-%>
knitr_insert_tabs()
```

<% has_known_anno = FALSE
if(!is.null(res_list@list[[1]]@anno)) { 
has_known_anno = TRUE -%>


```{r, echo = FALSE, message = FALSE}
message(qq("* testing correlation of subgroups to known annotations"))
```

Test correlation between subgroups and known annotations:

```{r, results = 'asis', echo = FALSE, include = TRUE}
<% for(k in all_k) { -%>
knitr_add_tab_item('test_to_known_factors(res_list, k = <%=k%>)', 'k = <%=k%>')
<%}-%>
knitr_insert_tabs()
```

<%}-%> 

## Results for each method

<% 
is_hc = FALSE
n_section = n_top_value_method * n_partition_method
i_section = 0
for(top_value_method in res_list@top_value_method) {
    for(partition_method in res_list@partition_method) { 
    	i_section = i_section + 1 -%>
    	<% brew(paste0(TEMPLATE_DIR, "/section_template.Rmd")) %>

<% }} -%>

## Session info

```{r}
sessionInfo()
```
