...
\arguments{
  ...
  \item{scale_method}{in some circumstances, data matrix are scaled by rows before sent to the partition function. The default scaling is apply by \code{\link[base]{scale}}. However, some partition function may not accept negative values which  are produced by \code{\link[base]{scale}}. Here \code{scale_method} can be set to \code{rescale} which scale rows by \code{(x - min)/(max - min)}. Here \code{scale_method} only means how to scale rows when \code{scale_rows} is set to \code{TRUE} in \code{\link{consensus_partition}} or \code{\link{run_all_consensus_partition_methods}}. The value for \code{scale_method} can be a vector.}
