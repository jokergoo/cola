...
%\VignetteIndexEntry{A Framework for Consensus and Hierarchical Partitioning}
-->
...
2. Extract subset of rows with highest scores. Here "scores" are calculated by
   ...
   `var()` or `sd()`). However the choice of "the top-value method" can be
...
4. Randomly sample a subset of rows from the matrix with probability $p$ and
   ...
   method, with trying different numbers of subgroups. Note the random sampling
   can also be applied on columns.
...
registered top-value methods, simply use `remove_top_value_method()` by
...
where `ConsensusPartition` class is for analysis of a single top-value method
and a single partition method, while `ConsensusPartitionList` class is for
multiplt top-value methods and multiplt partition methods.
...
- `get_stat()`: Extract the statistics for determining best number of subgroups.
...
```{r, eval = FALSE}
data = read.table("https://jokergoo.github.io/cola_examples/unifiedScaled.txt", 
