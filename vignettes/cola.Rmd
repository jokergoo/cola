...
analysis, especially for gene expression and methylation data analysis.
Mostly, unsupervised clustering methods are applied to predict new subgroups
or test the consistency with known annotations.
...
2. Extract subset of rows with highest scores. Here "scores" are calculated by
   ...
   analysis, $n$ rows with highest variance are used in most cases, where the
...
4. Randomly sample a subset of rows from the sub-matrix with probability $p$
   and perform partition on the columns of the matrix by a certain partition
...
both types of matrices, columns should always correspond to samples and
subgroups are detected on samples. More general, the input matrix can be any
...
"samples". **Note, users should apply proper normalization to the matrix
before performing cola analysis.**
...
- `partition_repeats`: times of random sampling and partitioning to perform.
...
These methods can be used in consensus partition by providing the name to the
...
with non-zero covariance have higher ATC values, even higher when the number
of correlated rows increases (although the correlation value itself is small,
bottom left figure). This shows ATC method can assign higher values for rows
which correlate to more other rows.
...
# code is only for demonstration
register_partition_methods(
    hclust_cor = function(mat, k) cutree(hclust(as.dist(1 - cor(mat))))
)
```
...
[**NNLM**](https://cran.r-project.org/package=NNLM) packages. Following is a
quick comparison of the two packages on [**the Golub
dataset**](http://bioconductor.org/packages/golubEsets/). The two heatmaps
show the __H__ matrix which is the meta-gene matrix (columns correspond to
samples) calculated by the two packages with rank 3. According to the
heatmaps, **NMF** package performs better than **NNLM** package because
**NMF** separates AML and ALL samples very nicely.
...
rows before partitioning. There are three possible values:

- `standardization`: z-score transformation, which is `(x - mean(x))/sd(x)`.
- `rescale`: `(x - min(x))/(max(x) - min(x))`, this ensures all the scaled
...
provides metrics to help to determine the best number of subgroups. [the
NbClust package](https://CRAN.R-project.org/package=NbClust) provides a nice
overview of determining best number of groups from clustering.
...
the ambiguous clustering**, which can be calculated by $F(x_2) - F(x-1)$.
...
PAC](https://www.nature.com/articles/srep06207), $x_1$ and $x_2$ are fixed
(0.1 and 0.9). But in many cases, the choice of $x_1$ and $x_2$ is quite
sensitive, thus, in **cola** package, the PAC implementation is a slightly
changed to its original implementation. For following form of PAC:

$$PAC_a = F(x_2) - F(x_1)$$

$x_1$ takes a list of values from $[0.1, 0.3]$ and $x_2$ takes a list of
values in $[0.7, 0.9]$. The final PAC score is the mean of $PAC_a$ by removing
the top 10^th and bottom 10^th values. This method improves the robustness of
...
**cola** provides a `guess_best_k()` function which suggests the best $k$.
...
`guess_best_k()` only gives suggestion on selecting a reasonable $k$.
...
[here](https://tcga-data.nci.nih.gov/docs/publications/gbm_exp/). The web page for the analysis is 
...
subgroups are found first, and secondary subgroups are detected afterwards.
...
first group contains samples with largest distance to all other subgroups and
the second groups are all other samples. For each of the two groups, if
the number of samples is less than 6, the hierarchical partition stops, or it
repeatedly performs the hierarchical partition on the subset of samples in the
...
`run_all_consensus_partition_methods()`) are added as an annotation to compare
...
`rh` is already generated and can be downloaded from [here](https://jokergoo.github.io/cola_examples/TCGA_GBM/TCGA_GBM_subgroup_hierarchical_partition.rds).
...
Simply typing `rh` gives the summary of the analysis.
...
On each node of the partition hierarchy, it is a `ConsensusPartition` object (note it is
...
`collect_classes()` plots the hierarchy of subgroups as well as the annotations 
that are set before.
...
`get_sigatures()` can also be applied to the `rh` object to visualize the signature rows
...
for `rh` can be found at https://jokergoo.github.io/cola_examples/TCGA_GBM/TCGA_GBM_subgroup_hierarchical_partition_cola_report/cola_hc.html.
...
Following functions can be applied to the `HierarchicalPartition` class:

- `cola_report()`: Generate a HTML report for the complete analysis.
- `collect_classes()`: Make heatmaps for the hierarchical partition.
- `dimension_reduction()`: Make PCA plot.
- `get_classes()`: Get class labels at a specific depth of the hierarchy.
- `get_signatures()`: Make the heatmap for the signatures. 
- `test_to_known_factors()`: Apply tests to the annotations if provides. The test will be Fisher's exact test or ANOVA depending on the data type of the annotation.
