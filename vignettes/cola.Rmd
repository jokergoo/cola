...
unsupervised clustering methods are applied to predict new subgroups or
test the consistency with known annotations.
...
sampled subset of data and summarizes the robustness of the clustering, and finally
...
All the steps will be explaied in detail in following sections.
...
- `top_value_method`: name of the method to assign scores to
  ...
  values.
- `top_n`: number of rows with top values used for partition. Normally we set
...
`run_all_consensus_partition_methods()` is a convinient way to try multiple
top-value methods and multiple partition methods simultaneously to see which combination of
methods gives the best prediction of subgroups.

In `run_all_consensus_partition_methods()`, most of the arguments are the same
as in `consensus_partition()`, except `top_value_method` and `partition_method` now
accept a vector of method names and multiple cores can be set by `mc.cores` argument.
...
`cola_report()` function can be applied on `rl` to generate a HTML
report for the complete analysis, with all the plots generated.

## Top-value methods

Top-value methods are used to assign scores to matrix rows, later the
...
These methods can be used in consensus partition by providing the name to the `top_value_method`
argument in `run_all_consensus_partition_methods()` or `consensus_partition()`.
...
the ATC (ability to correlate to others) for row $i$ is defined as:
...
only with very small correlation value. They will gain high ATC value due to
...
where now $ATC_i$ is the read area only on the right of $x = \alpha$. $\beta$
is the power added to the absolute correlations that it decreases more for the
smaller correlations. By Default $\alpha$ is set to 0 and $\beta$ is set to 1.

Next we perform a simulation test to show the attributes of ATC. A matrix with
160 rows, 100 columns with random values are generated as follows:

1. 100 rows with mean of 0, covariance matrix is set to 0 and 1 on the diagnal;
2. 10 rows with mean of 0, covariance matrix is set to 0.8 and 1 on the diagnal (high
   correlation but a small group of rows);
3. 50 rows wiht mean of 0, covariance matrix is set to 0.5 and 1 on the diagnal
...
calculating ATC scores. The bottom left figure is the ATC for all 160 rows and the
...
distinguished very well by using variance (bottom right figure). As a
contrast, the rows with covariance have higher ATC values, even higher when
the number of correlated rows increases (although the correlation value itself
is small, bottom left figure). This shows ATC method can assign higher values
for rows which correlate to more other rows.
...
- `hclust`: hierarchical clustering + cutree. The parameters for calling `hclust()` and `dist()` are
   all defaults for the two functions, thus it is Euclidean distance with "complete" clustering method.
...
need to be different for different classes. These labels will be re-coded
...
# code is only for demonstration
register_partition_methods(
    hclust_cor = function(mat, k) cutree(hclust(as.dist(cor(mat))))
)
```
...
`register_NMF()` functions. For NMF methods, there are two packages supported
in `register_NMF()` which are
[**NMF**](https://CRAN.R-project.org/package=NMF) (the default method) and
[**NNLM**](https://cran.r-project.org/package=NNLM) packages. Following is a quick
comparison of the two packages on [**the Golub dataset**](http://bioconductor.org/packages/golubEsets/). The two heatmaps show the __H__ matrix which is the meta-gene matrix (columns correspond to samples) calculated by the two packages with rank 3.
According to the heatmaps, **NMF** package performs better than **NNLM** package
because **NMF** separates AML and ALL samples very nicely.

<img src="NMF_compare.png" />
...
Assume there is a stable subgroup, which means, for any pair of samples both
in that subgroup, $c_{ij}$ is close to 1, and for any sample $j$ which is not
in that subgroup, $c_{ij}$ is close to 0. Thus, if the consensus matrix is
visualized as a heatmap, the samples in the subgroup will be represented as a
block in the diagnal of the heatmap.
...
between two sets of labels to maximize $\sum I(s_{1i}, m(s_{2i}))$ where $s_1$ is
...
additionally adjusted by the mean distance in each subgroup (calculated
from the original matrix), which means, the subgroup with label 1 always has
the smallest mean intra-group distance.
...
annotations). This might indicate these are the subset of samples which are like in
intermediate state between group 1 and group 2.
...
For the topic of determining best number of clustering, please refer to [the NbClust package](https://CRAN.R-project.org/package=NbClust).
...
subgroups (an example is [here](#toc_26)).
...
In the original implementation of PAC, $x_1$ and $x_2$ are fixed (0.1 and
...
where $N_s$ is the number of samples and $sc$ is the consensus subgroup label. Note
class labels in single partitions have already been adjusted to the consensus
partition labels. 
...
Note the number of subgroups that can be inferred from the consensus heatmap is not
necessary to be exactly the same as $k$. It can be smaller than $k$.
...
`guess_best_k()` only gives suggestion of selecting a reasonable $k$.
...
- `samr` and `pamr`: use [SAM](https://CRAN.R-project.org/package=samr)/[PAM](https://CRAN.R-project.org/package=pamr) method to find significantly different rows between
  subgroups.

Users can also provide their own method by providing a function with the matrix and subgroup labels
as inputs and a vector of FDR as output.

## Implementation of the package

**cola** is implemented in a object-oriented way. There are two main classes
...
demonstrated [here](https://jokergoo.github.io/cola_examples/) (Running cola analysis
...
partition method. In most cases, we have no idea of which combination
...
visualizes how subgroups are corresponded with increasing $k$. Each row in 
...
"best k" is less than 0.2, the samples are split into two subgroups where the
first group contains samples with smallest intra-group mean distance and
the second groups are all other samples. For each of the two groups, if
the number of samples is less than 6, the hierarchial partition stops, or it
repeatedly performs the hierarchial partition on the subset of samples in the
...
`rh` is already generated and can be downloaded from [here](https://jokergoo.github.io/cola_examples/TCGA_GBM_subgroup_hierarchical_partition.rds).
...
excluding the last digit is the name node of the parent node. E.g. for the
node `0011`, the depth is 4 and the parent node is `001`. For each node, the
last digit represents the index of the subgroup detected in parent node. and 0
represents all other subgroups. E.g. `02` means this node contains samples
...
to other subgroups), and `00` contains samples which do not belong to subgroup
2 at node 0.

`collect_classes()` plots the hierarchial of subgroups as well as the annotations 
that are set before.
...
subgroups. However, if we directly check the 5 subgroups in `rl`, actually the
