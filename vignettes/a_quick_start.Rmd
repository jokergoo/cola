...
1. Adjust the matrix. In this step, rows with too many `NA`s are removed. Rows
   ...
   optional.
2. Run consensus partitioning with multiple methods. The default partition
   methods are `hclust`, `kmeans`, `skmeans::skmeans`, `cluster::pam` and
   `Mclust::mclust`. The default methods to extract top n rows are `SD`, `CV`,
   `MAD` and `ATC`.
3. Generate a detailed HTML report for the complete analysis.

There are examples on real datasets for cola analysis that can be found at https://jokergoo.github.io/cola_examples/ and https://jokergoo.github.io/cola_collection/.
