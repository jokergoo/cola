...
hierarchical_partition = function(data, column_index = seq_len(ncol(data)), reduce = 0.5, PAC_cutoff = 0.2, min_samples = 6, ...) {
	
	.h_obj = new.env()
	.h_obj$hierarchy = matrix(nrow = 0, ncol = 2)
	.h_obj$list = list()

	.hierarchical_partition = function(data, column_index = seq_len(ncol(data)), reduce = 0.5, PAC_cutoff = 0.2, 
	...
	.hierarchical_partition(data, column_index = column_index, reduce = reduce, PAC_cutoff = PAC_cutoff, 
